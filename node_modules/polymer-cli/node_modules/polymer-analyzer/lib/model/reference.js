...
class ScannedReference extends feature_1.ScannedFeature {
    ...
    resolveWithKind(document, kind) {
        ...
        const scopedResult = resolveScopedAt(this.astPath, this.identifier, document, kind);
...
function resolveScopedAt(path, identifier, document, kind) {
    ...
    if (hasASingleDotInName.test(identifier)) {
        ...
        if (namespaceBinding !== undefined) {
            ...
            if (babel.isImportNamespaceSpecifier(node)) {
                return resolveThroughImport(namespaceBinding.path, name, document, kind);
            }
        }
    }
...
function resolveThroughImport(path, exportedAs, document, kind) {
    const statement = esutil.getCanonicalStatement(path);
    if (statement === undefined) {
    ...
    return resolveScopedAt(export_.astNodePath, exportedAs, import_.document, kind);
}
...
function getExportedIdentifier(node, localIdentifier) {
    switch (node.type) {
        ...
        case 'ExportNamedDeclaration':
            ...
            throw new Error(`Internal error: could not find import specifier for '${localIdentifier}'. Please report this bug.`);
