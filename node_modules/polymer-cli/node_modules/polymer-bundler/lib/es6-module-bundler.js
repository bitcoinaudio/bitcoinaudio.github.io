...
function prepareBundleModule(bundler, manifest, assignedBundle) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const sourceUrl of [...assignedBundle.bundle.files].sort()) {
            const rebasedSourceUrl = url_utils_1.ensureLeadingDot(bundler.analyzer.urlResolver.relative(url_utils_1.stripUrlFileSearchAndHash(assignedBundle.url), sourceUrl));
            const moduleDocument = analyzer_utils_1.getAnalysisDocument(sourceAnalysis, sourceUrl);
            const moduleExports = es6_module_utils_1.getModuleExportNames(moduleDocument);
            const starExportName = es6_module_utils_1.getOrSetBundleModuleExportName(assignedBundle, sourceUrl, '*');
            ...
            if (moduleExports.size > 0) {
                ...
                bundleSource.body.push(babel.exportNamedDeclaration(undefined, [...moduleExports].map((e) => babel.exportSpecifier(babel.identifier(e), babel.identifier(es6_module_utils_1.getOrSetBundleModuleExportName(assignedBundle, sourceUrl, e)))), babel.stringLiteral(rebasedSourceUrl)));
            }
        }
